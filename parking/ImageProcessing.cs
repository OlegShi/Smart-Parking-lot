...
namespace parking
{
    class ImageProcessing
    {
        ...
        public void saveToFile(string hexString, string path, string filename)
        {
            try
            {
                string fileName = path + filename;
                if (!File.Exists(fileName))
                    File.WriteAllBytes(fileName, Hex2Bin(hexString));
            }
            catch (Exception ex)
            {
        ...
        public void saveToFile(byte[] data, string path, string filename)
        {
            try
            {
                string fileName = path + filename;
